...
| `termux/img2claude` | Termux (Android) script: sends an image (photo/screenshot) into the Claude session running on the desktop. `scp` to the desktop, then **PowerShell `SetImage` into the Windows clipboard** (not Wayland: Claude v2.1.152 only queries `image/png`/`image/bmp` via `wl-paste`, never `image/jpeg`, so `wl-copy` left the paste empty — the PowerShell `Get-Clipboard -Format Image` fallback accepts every source format), then `tmux send-keys M-v`. Two entry points: with a file argument (called by the share hook) or no-arg (sends the most recent — bound to the `i` shortcut). Works around mosh not carrying the clipboard (Alt+V can't cross it). |
