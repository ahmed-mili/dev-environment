...
| **F2** — desktop WSL shell | `all` (default) | everything (vaults open as Windows Terminal tabs running native Zellij via interop) |
...
> **Picking a vault from the phone opens native Windows PowerShell — automatically.** A vault lives on `C:`, so it must run in native `pwsh`, but the phone's SSH-into-WSL session **can't reach Windows**: the **WSL→Windows hop is broken by a WSL mirrored-networking bug** — `127.0.0.1` is policy-routed to `loopback0` and the host handshake times out (confirmed across *all* Windows ports; `LoopbackEnabled=True` + `wsl --shutdown` does **not** fix it). So when you pick a vault in `pwsh`, the sessionizer (in WSL) records the name and exits with code 42; the phone then opens the vault client-side. The robust path is **not** `ssh -> Windows -> zellij attach`: Windows OpenSSH can put each reconnect in a different Zellij IPC/window-station scope, which looks active in metadata but resurrects as a fresh shell. Instead Termux runs the **local Zellij client** and reaches the Windows Zellij web server through an SSH tunnel: `127.0.0.1:<local> -> desktop:127.0.0.1:8082`. A token lives only in `~/.config/pc-zellij-web-token` on the phone. If Wi-Fi/5G drops, the tunnel dies, but the Windows Zellij web session keeps the pane alive; running `pwsh` again creates a new tunnel and reattaches to the same live session. The Windows PowerShell profile auto-`cd`s into `C:\obsidian-vaults\<session>` when the Zellij session name matches a vault, so new web-created panes still land in the right folder. No automatic `delete-session` runs on open; only explicit `Ctrl+X` is destructive. The Windows account for the SSH tunnel can't be derived from the phone, so it's read from **`$WIN_USER`** — set it in `~/.bashrc.local` (`export WIN_USER=<your Windows account>`), kept out of git so the versioned `bashrc` stays shareable. One-time Windows setup: install Termux `zellij`, start `zellij web --daemonize --ip 127.0.0.1 --port 8082` on Windows, create a web token with `zellij web --create-token`, and store the token in `~/.config/pc-zellij-web-token` on the phone.
