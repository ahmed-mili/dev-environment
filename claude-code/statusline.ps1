...
# banner du path. Meme parade que le sessionizer (ConvertTo-ArabicDisplay dans
# claude-code/windows-sessionizer/sessionizer.ps1, tests + justification la-bas,
# et port Rust identique dans statusline-rs/src/main.rs) : convertir les runs
# arabes en formes de presentation Unicode (U+FExx) posees en ordre visuel.
# Applique au seul chemin AFFICHE -- la detection git recoit le chemin brut.
$script:ArForms = @{
...
# cookie Firefox. Rafraîchi toutes les ~60s en détaché.
...
if ($isOllama -and (-not $usage)) {
    ...
    # ollama-usage.py est best-effort : en cas d'échec
